<?php
if( !function_exists('pd_is_attachment_alt') ){
	function pd_is_attachment_alt( $attachment_id ){
		if ( ! $attachment_id ) {
			return '';
		}
...
if( !function_exists('pd_is_get_post_types') ){
	function pd_is_get_post_types(){
		$args = array(
		   'public'   => true,
		   '_builtin' => false
		);
...
if( !function_exists('pd_is_get_post_status') ){
	function pd_is_get_post_status(){
		$post_statuses = array();
...
if( !function_exists('pd_is_get_post_lists') ){
	function pd_is_get_post_lists($post='post'){
		$post_lists = array();
...
if( !function_exists('pd_is_get_excerpt') ){

	function pd_is_get_excerpt( $args = array() ) {
		...
		// Apply filters
		$defaults = apply_filters( 'pd_is_get_excerpt_defaults', $defaults );

		// Parse args
		$args = wp_parse_args( $args, $defaults );

		// Apply filters to args
		$args = apply_filters( 'pd_is_get_excerpt_args', $args );
		...
		return apply_filters( 'pd_is_get_excerpt', $output );
	}
}