...
class Ajax
{
	...
	{
		add_action('wp_ajax_nopriv_pd_is_load_posts', [$this, 'pd_is_load_posts']);
		add_action('wp_ajax_pd_is_load_posts', [$this, 'pd_is_load_posts']);
	}

	public function pd_is_load_posts(){
		        ...
		        if( $post_query->have_posts() ){
					...
					while( $post_query->have_posts() ){
						...
						if( $template_style === 'default' ){
							...
							require( PD_IS_PATH . 'templates/style-2/template.php' );
						}
					}
					// echo $offset;
					// echo '<br>'.$post_query->found_posts;
					if( ($settings['offset']+$post_query->post_count) >= $post_query->found_posts ){
						echo '<span class="pd_is_reach_limit pd-is-d-none"></span>';
					}else{
					}
					wp_reset_postdata();
				}
