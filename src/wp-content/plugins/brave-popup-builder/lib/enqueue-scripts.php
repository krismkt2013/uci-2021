...
function bravepop_enqueue_front_scripts() {
	if ( !is_admin() ) {
      ...
      $customFonts = isset($currentSettings['fonts']) ? $currentSettings['fonts'] : array();
...
// function bravepop_embedlock_script() {   
//    global $post;   
//    if( isset($post->post_content) && has_shortcode( $post->post_content, 'bravepop') )   {     
//       wp_enqueue_script( 'bravepop_embedlock', BRAVEPOP_PLUGIN_PATH . 'assets/frontend/embedlock.js' ,'','',false);
//    } 
// } 
// add_action( 'wp_enqueue_scripts', 'bravepop_embedlock_script');
...
function bravepop_popupjs_vars() { 
   ...
   $emailValidator = isset($settings['emailvalidator']->active) && $settings['emailvalidator']->active !== 'disabled' ? true : false;
