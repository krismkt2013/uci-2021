...
function bravepop_get_wpdata( $type ) {
   ...
   if($type == 'all'){
       ...
       //GET ALL PAGES
       $pages = get_pages(array('numberposts' => 100, 'suppress_filters' => true, 'lang' => '')); 
       ...
       if ( BRAVEPOP_WOO_ACTIVE) {
           ...
           $products = wc_get_products( array( 'numberposts' => -1, 'suppress_filters' => true, 'status' => 'publish', 'lang' => ''));
   ...
   if($type == 'media'){
       $allMedia = [];
       $attachments = get_posts( array( 'post_type' => 'attachment', 'numberposts' => 200,'post_status' => null,'post_parent' => null) );
       foreach ( $attachments as $attachment ) {
           $object = new stdClass();
           $object->ID = $attachment->ID ;
           $object->title = $attachment->post_title;
